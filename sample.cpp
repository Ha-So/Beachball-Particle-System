...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Haris Sohail" };
...
const char *CL_FILE_NAME   = { "sohailh18.cl" };
...
Animate( )
{
	...
	status = clEnqueueAcquireGLObjects(CmdQueue, 1, &dP2obj, 0, NULL, NULL);
	PrintCLError(status, "clEnqueueAcquireGLObjects (3): ");
	status = clEnqueueAcquireGLObjects(CmdQueue, 1, &dP3obj, 0, NULL, NULL);
	PrintCLError(status, "clEnqueueAcquireGLObjects (4): ");
	...
	PrintCLError( status, "clEnqueueReleaseGLObjects (2): " );
	status = clEnqueueReleaseGLObjects(CmdQueue, 1, &dP2obj, 0, NULL, NULL);
	PrintCLError(status, "clEnqueueReleaseGLObjects (3): ");
	status = clEnqueueReleaseGLObjects(CmdQueue, 1, &dP3obj, 0, NULL, NULL);
	PrintCLError(status, "clEnqueueReleaseGLObjects (3): ");
	Wait( CmdQueue );
	clFinish(CmdQueue);
...
Display( )
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, hP2obj);
	glVertexPointer(4, GL_FLOAT, 0, (void *)0);
	glEnableClientState(GL_VERTEX_ARRAY);

	glBindBuffer(GL_ARRAY_BUFFER, hP3obj);
	glVertexPointer(4, GL_FLOAT, 0, (void *)0);
	glEnableClientState(GL_VERTEX_ARRAY);
...
InitCL( )
{
	...
	glGenBuffers(1, &hP2obj);
	glBindBuffer(GL_ARRAY_BUFFER, hP2obj);
	glBufferData(GL_ARRAY_BUFFER, 4 * NUM_PARTICLES * sizeof(float), NULL, GL_STATIC_DRAW);

	glGenBuffers(1, &hP3obj);
	glBindBuffer(GL_ARRAY_BUFFER, hP3obj);
	glBufferData(GL_ARRAY_BUFFER, 4 * NUM_PARTICLES * sizeof(float), NULL, GL_STATIC_DRAW);
	...
	dP2obj = clCreateFromGLBuffer(Context, 0, hP2obj, &status);
	PrintCLError(status, "clCreateFromGLBuffer (3)");

	dP3obj = clCreateFromGLBuffer(Context, 0, hP3obj, &status);
	PrintCLError(status, "clCreateFromGLBuffer (4)");
...
#define EXPORT_BINARY
	...
	status = clSetKernelArg(Kernel, 1, sizeof(cl_mem), &dP2obj);
	PrintCLError(status, "clSetKernelArg (1): ");

	

	status = clSetKernelArg( Kernel, 2, sizeof(cl_mem), &dVel );
	PrintCLError( status, "clSetKernelArg (2): " );

	status = clSetKernelArg( Kernel, 3, sizeof(cl_mem), &dCobj );
	PrintCLError( status, "clSetKernelArg (3): " );

	status = clSetKernelArg(Kernel, 4, sizeof(cl_mem), &dP3obj);
	PrintCLError(status, "clSetKernelArg (1): ");
}
...
InitLists( )
{
	...
	glNewList( SphereList, GL_COMPILE );
		glColor3f( .9f, .9f, 0. );
		glPushMatrix( );
			glTranslatef( -500., -25000., 0. );
			glutWireSphere( 15000., 100, 100 );
		glPopMatrix( );
		glColor3f(.5f, .5f, 0.9f);
		glPushMatrix();
		...
		glPushMatrix();
			glTranslatef(-100, -5000., 0.);
			glutWireSphere(5000., 100, 100);
		glPopMatrix();
	glEndList( );
...
ResetParticles( )
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, hP2obj);
	struct xyzw *points2 = (struct xyzw *) glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
	for (int i = 0; i < NUM_PARTICLES; i++)
	{
		points2[i].x = Ranf(XMIN, XMAX);
		points2[i].y = Ranf(YMIN, YMAX);
		points2[i].z = Ranf(ZMIN, ZMAX);
		points2[i].w = 1.;
	}
	glUnmapBuffer(GL_ARRAY_BUFFER);

	glBindBuffer(GL_ARRAY_BUFFER, hP3obj);
	struct xyzw *points3 = (struct xyzw *) glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
	for (int i = 0; i < NUM_PARTICLES; i++)
	{
		points3[i].x = Ranf(XMIN, XMAX);
		points3[i].y = Ranf(YMIN, YMAX);
		points3[i].z = Ranf(ZMIN, ZMAX);
		points3[i].w = 1.;
	}
	glUnmapBuffer(GL_ARRAY_BUFFER);
...
Quit( )
{
	...
	clReleaseMemObject(		dP2obj	);
	clReleaseMemObject(		dP3obj	);
...
WriteFrame( )
{
		...
		glBindBuffer(GL_ARRAY_BUFFER, hP2obj);
		struct xyzw *points2 = (struct xyzw *) glMapBuffer(GL_ARRAY_BUFFER, GL_READ_ONLY);
		for (int i = 0; i < NUM_PARTICLES; i++)
		{
		...
		glBindBuffer(GL_ARRAY_BUFFER, hP3obj);
		struct xyzw *points3 = (struct xyzw *) glMapBuffer(GL_ARRAY_BUFFER, GL_READ_ONLY);
		for (int i = 0; i < NUM_PARTICLES; i++)
		{
